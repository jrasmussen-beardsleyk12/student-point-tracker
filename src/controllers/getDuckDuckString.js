...
module.exports = {
  ...
  params: {
    duckString: (context, req) => {
      return context.query.duckString(req);
    },
  },
  ...
  async logic(req, res, context) {
    ...
    let duckData = context.globalCache.find(
      `duck-svg__${params.duckString}`,
      () => {
        ...
        return context.globalCache.add(
          `duck-svg__${params.duckString}`,
          duckSVG,
        );
      },
    ...
    res.set(
      "Cache-Control",
      `must-revalidate, public, max-age=${context.config.MAX_AGE_DUCKS}`,
    );
